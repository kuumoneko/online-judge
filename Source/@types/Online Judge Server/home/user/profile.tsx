...
import { all_language, color, get_rank_color, getdata, getGravatarURL ,Theme_mode , Languages , User_role , User} from "online-judge-types";
...
export function Profile({ user }: { user: User }) {
    ...
    useEffect(() => {
        async function lmao() {
            ...
            const rank_by_points = await getdata("sort", "users", { mode: "points", search: { search: user.username }, reverse: true, page: 1, lineperpage: 100 })
            const rank_by_rank = await getdata("sort", "users", { mode: "rank", search: { search: user.username }, reverse: true, page: 1, lineperpage: 100 })
