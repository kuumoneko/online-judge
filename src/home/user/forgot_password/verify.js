...
async function lmao() {
    ...
    // console.log(res)
    await getdata("verify_forgot_password", "", { username: res.data.data[0].username, mode: "code", data: undefined });
    return {
...
export function Verify_password_page() {
    ...
    useEffect(() => {
        async function lmao() {
            ...
            // console.log(code)
            const res = await getdata("verify_forgot_password", "", { username: username, mode: "code", data: code });
    ...
    return (React.createElement("div", { style: { display: "flex", flexDirection: "column" }, onKeyDown: (e) => {
        ...
        React.createElement("div", { style: { display: "flex", flexDirection: "row", justifyContent: "space-evenly" } }, Array(6)
            .fill(0)
            .map((_, index) => (React.createElement("input", { key: index, id: `codeInput-${index}`, type: "text", maxLength: 1, 
            // onPaste={handlePaste}
