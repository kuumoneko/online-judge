export var User_role;
(function (User_role) {
    User_role["administrator"] = "admin";
    User_role["contributor"] = "cont";
    User_role["moderator"] = "mod";
    User_role["user"] = "user";
})(User_role || (User_role = {}));
export var Theme_mode;
(function (Theme_mode) {
    Theme_mode["dark"] = "dark";
    Theme_mode["light"] = "light";
})(Theme_mode || (Theme_mode = {}));
export var UI_getting_mode;
(function (UI_getting_mode) {
    UI_getting_mode["fonr"] = "font";
    UI_getting_mode["background"] = "background";
    UI_getting_mode["content"] = "content";
})(UI_getting_mode || (UI_getting_mode = {}));
export var Languages;
(function (Languages) {
    Languages["C03"] = "C++03";
    Languages["C11"] = "C++11";
    Languages["C14"] = "C++14 (C++ Themis)";
    Languages["C17"] = "C++17";
    Languages["C20"] = "C++20";
    Languages["PY3"] = "Python 3";
    Languages["JAVA"] = "Java";
    Languages["JS"] = "Javascript";
})(Languages || (Languages = {}));
export var Coding_status;
(function (Coding_status) {
    Coding_status["AC"] = "Accept";
    Coding_status["WA"] = "Wrong Answer";
    Coding_status["TLE"] = "Time Limit Error";
    Coding_status["RTE"] = "Run Time Error";
    Coding_status["CE"] = "Compile error";
    Coding_status["OLE"] = "Output Limit Error";
    Coding_status["MLE"] = "Memory Limit Error";
    Coding_status["SE"] = "System Error";
})(Coding_status || (Coding_status = {}));
//# sourceMappingURL=enum.js.map