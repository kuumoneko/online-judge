...
import { getdata, User } from "online-judge-types";


async function lmao(url: string[]) {
    const res: User = await getdata("get", "users", url[2]);
    // setemail(res.email)
    // setusername(res.username as string)
    // console.log(res)
    await getdata("verify", "", { username: res.username, code: undefined });
...
export function Verify_user({ url }: { url: string[] }) {
                ...
                {Array(6)
                    ...
                    .map((_, index) => (
                        <input
                            ...
                            style={{ height: "40px", width: "30px", margin: "10px", color: "black" }}
                        />
                    ))}
            </div>
